...
puts "Found the following apps to backup: #{databases.inspect}"

apps = Dir.entries(AppsPath).select {|d| d != '.' && d != '..'}
...
Backup::Model.new(:databases, 'My Backup') do
  ...
  apps.each do |appname|
    path = File.join(AppsPath,appname,'shared','system')
    if File.directory? path
      sync_with S3 do |s3|
        s3.access_key_id      = S3Key
        s3.secret_access_key  = S3Secret
        s3.bucket             = S3Bucket
        s3.path               = File.join(S3Path,appname)
        s3.mirror             = true
#        s3.additional_options = ['--exclude="/bundle\/|git_cache\/"']
